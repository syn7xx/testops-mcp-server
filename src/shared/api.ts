...
export const apiFetch = async <T = unknown>(
    ...
    return tryCatch(async () => {
        ...
        if (!response.ok) {
            throw new Error(`${config.method} ${endpoint} failed: ${response.status}`);
        }
...
): Promise<Result<T, Error>> => apiFetch<T>(endpoint, { method: 'GET', queryParams });
...
export const apiPost = <T = unknown>(
    ...
    body?: unknown,
    queryParams?: RequestConfig['queryParams'],
): Promise<Result<T, Error>> => apiFetch<T>(endpoint, { method: 'POST', body, queryParams });
...
export const apiPatch = <T = unknown>(
    ...
    body?: unknown,
    queryParams?: RequestConfig['queryParams'],
): Promise<Result<T, Error>> => apiFetch<T>(endpoint, { method: 'PATCH', body, queryParams });

/** DELETE request helper */
export const apiDelete = <T = unknown>(
    endpoint: string,
    queryParams?: RequestConfig['queryParams'],
): Promise<Result<T, Error>> => apiFetch<T>(endpoint, { method: 'DELETE', queryParams });
